...
export function buildTelegramStatementExtractionPrompt(
  ...
  return [
    ...
    "- 요구 문장만 대상이 아니다. 성명/논평/입장 라벨이 있는 글에서 제목이나 부제가 단체의 판단, 비판, 우려, 연대, 다짐을 선명하게 담으면 displayable=true로 볼 수 있다.",
    "- 제목이나 부제를 고를 때도 원문에 실제로 있는 연속 문자열을 그대로 복사한다. 숫자, 가운데점, 쉼표, 느낌표, 띄어쓰기, 대괄호 라벨을 임의로 바꾸지 않는다.",
    ...
    "- '여전히 타는 목마름으로, 정치장벽 너머 민주주의로!' => displayable=true, sentence_role=pledge",
